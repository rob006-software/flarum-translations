# Translations inherited from Flarum 1.x

Translations for **87** languages are inherited from Flarum 1.x, but they can be adjusted
independently after inheritance. Reports below list all strings which have the same source string on both
sides, but do not match between them - **24** languages have such strings, the remaining ones
are fully in sync and are not listed here.

| Language | Different translations | Missing translations | Components |
| --- | --- | --- | --- |
| [Azerbaijani](az.md) (`az`) | 0 | 2 | 1 |
| [Chinese (Simplified)](zh_Hans.md) (`zh_Hans`) | 1901 | 1827 | 138 |
| [Chinese (Traditional)](zh_Hant.md) (`zh_Hant`) | 33 | 199 | 6 |
| [Croatian](hr.md) (`hr`) | 249 | 33 | 1 |
| [Dutch](nl.md) (`nl`) | 0 | 9 | 8 |
| [Finnish](fi.md) (`fi`) | 5 | 61 | 7 |
| [French](fr.md) (`fr`) | 139 | 317 | 51 |
| [German](de.md) (`de`) | 322 | 577 | 94 |
| [German (formal)](de@formal.md) (`de@formal`) | 367 | 577 | 95 |
| [Hungarian](hu.md) (`hu`) | 44 | 6 | 22 |
| [Indonesian](id.md) (`id`) | 1 | 0 | 1 |
| [Italian](it.md) (`it`) | 0 | 1 | 1 |
| [Japanese](ja.md) (`ja`) | 1 | 0 | 1 |
| [Korean](ko.md) (`ko`) | 0 | 1 | 1 |
| [Persian](fa.md) (`fa`) | 9 | 175 | 62 |
| [Polish](pl.md) (`pl`) | 5 | 5 | 5 |
| [Romanian](ro.md) (`ro`) | 0 | 12 | 1 |
| [Russian](ru.md) (`ru`) | 57 | 61 | 17 |
| [Serbian (Latin)](sr_Latn.md) (`sr_Latn`) | 0 | 5 | 1 |
| [Spanish](es.md) (`es`) | 14 | 203 | 34 |
| [Spanish (formal)](es@formal.md) (`es@formal`) | 11 | 203 | 33 |
| [Swedish](sv.md) (`sv`) | 195 | 714 | 30 |
| [Turkish](tr.md) (`tr`) | 6 | 0 | 5 |
| [Vietnamese](vi.md) (`vi`) | 3 | 0 | 1 |
